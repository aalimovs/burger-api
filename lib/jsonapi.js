...
internals.replyJsonapi = function (response, options) {
	...
	if (sequelizeModel && sequelizeModel.dataValues && sequelizeModel.$modelOptions && sequelizeModel.$modelOptions.sequelize && !sequelizeModel.$options.isNewRecord) {
		response = new JSONAPISerializer(sequelizeModel.$modelOptions.name.singular, {
			attributes: sequelizeModel.jsonApiAttributes(),
		}).serialize(response);
	}

	// for newly created records http://jsonapi.org/format/#crud-creating-responses-201
	if (sequelizeModel.$options.isNewRecord) {
		const attributes = Object.keys(sequelizeModel.dataValues);
		if(attributes[0] === 'id') {
			delete attributes.shift(); // remove id
		}
		response = new JSONAPISerializer(sequelizeModel.$modelOptions.name.singular, {
	...
	return Joi.validate(response, internals.jsonapiDocument, (err) => {
		...
		// for newly created records http://jsonapi.org/format/#crud-creating-responses-201
		if (sequelizeModel.$options.isNewRecord) {
			response.data.links = response.links;
			response.data.links.self += '/' + sequelizeModel.id;
